...
def fetch_data(fred_key, days=90):
    ...
    try:
        ...
        for ticker, name in tickers.items():
            try:
                ticker_df = yf.download(ticker, start=start, end=end, progress=False)
                if not ticker_df.empty and 'Close' in ticker_df.columns:
                    data[name] = ticker_df['Close'].squeeze()
            except Exception as e:
                errors.append(f"{name}: {str(e)}")
...
st.markdown('<div class="core-metrics">', unsafe_allow_html=True)
...
for idx, (name, emoji, unit) in enumerate(core_metrics):
    col = cols[idx % 2]
    ...
    if name in df.columns:
        ...
        if len(series) > 0:
            ...
            with col:
                st.markdown(
                    f'<div class="core-item">'
                    f'<div class="core-label">{emoji} {name}</div>'
                    f'<div class="core-value">{current:.2f}{unit}</div>'
                    f'<div class="core-change" style="color: {"#27ae60" if change_pct > 0 else "#e74c3c" if change_pct < 0 else "#95a5a6"};">'
                    f'{"↑" if change_pct > 0 else "↓" if change_pct < 0 else "→"} {abs(change_pct):.2f}% 30日'
                    f'</div></div>',
                    unsafe_allow_html=True
                )
...
if '黄金价格' in df.columns and 'VIX恐慌指数' in df.columns:
    ...
    fig1.add_trace(
        go.Scatter(
            x=common_idx, y=gold_aligned, name="黄金",
    ...
    fig1.add_trace(
        go.Scatter(
            x=common_idx, y=vix_aligned, name="VIX",
...
if '美元指数' in df.columns and '实际利率' in df.columns:
    ...
    fig2.add_trace(
        go.Scatter(
            x=common_idx, y=dxy_aligned, name="美元指数",
    ...
    fig2.add_trace(
        go.Scatter(
            x=common_idx, y=rate_aligned, name="实际利率",
...
for name, unit, emoji, color in metrics:
    if name in df.columns:
        ...
        fig.add_trace(go.Scatter(
            x=chart_data['日期'], y=chart_data['数值'],
            ...
            hovertemplate=(
                '<b>%{x|%Y-%m-%d}</b><br>' +
                f'{name}: %{{y:.2f}}{unit}<br>' +
                '相对30日前: %{customdata:+.2f}%<extra></extra>'
            ),
...
with col1:
    ...
    st.download_button("📥 导出数据", csv, f"gold_{datetime.now():%Y%m%d}.csv", "text/csv", use_container_width=True)
